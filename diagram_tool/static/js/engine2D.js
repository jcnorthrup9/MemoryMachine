 ...
 *
 * Forked 2026-07-14 from static/js/engine2D.js (the original Digital
 * Palimpsest app) into this standalone diagram-authoring tool -- 3D scene
 * code and AI-hallucinate generation were dropped entirely; Pershing
 * Metabolizer already owns all 3D, and this tool's whole purpose is
 * reliable, exact zone-shape placement, not AI generation.
 */
const Engine2D = {
  /**
  ...
  render() {
    ...
    // 2. THE INTERVENTION: Draw the placed layer stack from MemoryState
      ...
      MemoryState.stack.forEach(item => {
        ...
        if (layerG) {
          ...
          xformed.querySelectorAll('path, polyline, line, polygon, rect, circle').forEach(el => {
            ...
            // Solid fills always -- fixed 2026-07-14. The original app set
            // fill-opacity:0.3 here (for on-screen layering legibility when
            // shapes overlapped), and that opacity survived unchanged into
            // every exported SVG (handleExport() only restores stroke/fill
            // color from data-orig-color, never resets fill-opacity). That
            // 30% dilution is what made every zone fill this tool ever
            // exported read as near-black to Pershing Metabolizer's color
            // classifier. This tool's whole purpose is reliable, exact zone
            // data -- solid fill, no exceptions, no separate "export mode"
            // to keep in sync.
            if (hadFill) {
