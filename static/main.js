...
// ── GLB LOADER ────────────────────────────────────────────────────────────────
function loadGLBIntoScene(glbUrl, svgX, svgZ, zoneType = '', stackId = null) {
  if (!threeScene) return;
              ...
              if (modelUrl) {
                if (result.stl_url && !result.glb_url) {
                  loadSTLIntoScene(modelUrl, pos.x, pos.z, seed.layerId);
                    ...
                    loadGLBIntoScene(modelUrl, pos.x, pos.z, seed.layerId, seed.id);
                }
...
function refreshStackUI() {
  ...
  list.innerHTML = visibleItems.map(item => {
    ...
    const collisionClass = item.isColliding ? ' collision-warning' : '';
    const collisionIcon  = item.isColliding ? `<div class="collision-tooltip">⚠ OVERLAP</div>` : '';
    ...
    return `
    <div class="stack-item${MemoryState.editingId === item.id ? ' selected' : ''}${dimClass}${lockClass}${collisionClass}"
         data-id="${item.id}">
...
function _wireSliders() {
  function _update3DObject(item) {
    if (!threeScene) return;
    threeScene.traverse(obj => {
      if (obj.userData.stackId === item.id) {
        const halfW = PARK_RECT_WIDTH / 2;
        const halfD = PARK_RECT_DEPTH / 2;
        const localX = Math.max(-0.85, Math.min(0.85, item.transform.x / SVG_OFFSET_MAX_X));
        const localZ = Math.max(-0.85, Math.min(0.85, item.transform.y / SVG_OFFSET_MAX_Z));
        const cos = Math.cos(PARK_ROTATION_Y);
        const sin = Math.sin(PARK_ROTATION_Y);
        
        // Reposition in world space mapping 2D layout to the rotated 3D Park bounds
        obj.position.x = PARK_CENTER_X + (localX * halfW * cos + localZ * halfD * sin);
        obj.position.z = PARK_CENTER_Z + (-localX * halfW * sin + localZ * halfD * cos);
        
        if (obj.userData.baseScale) {
          obj.scale.setScalar(obj.userData.baseScale * item.transform.scale);
        }
        if (obj.userData.baseRot !== undefined) {
          obj.rotation.y = obj.userData.baseRot - (item.transform.rot * Math.PI / 180);
        }
      }
    });
  }
