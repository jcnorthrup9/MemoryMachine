...
def get_vision_prompt(site_name):
...
for filename in os.listdir(ASSETS_DIR):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')):
        ...
        normalized_name = site_name.lower().replace("_", "").replace(" ", "")
        location = SITE_LOCATIONS.get(normalized_name, "")
        ...
        if not os.path.exists(spatial_path):
            ...
            try:
                ...
                response = ai_client.models.generate_content(model='gemini-2.5-flash', contents=[img, get_vision_prompt(site_name)])
        ...
        if not os.path.exists(review_path):
            ...
            try:
                ...
                search_query = f"{site_name} {location} park yelp tripadvisor google reviews".strip()
                ...
                review_prompt = f"You are an architectural researcher. I am giving you raw scraped markdown from a web search containing Yelp, TripAdvisor, and Google Maps snippets for \"{site_name}\" located in {location if location else 'its respective city'}. Extract any visitor reviews, sentiments, or descriptions of the park's atmosphere, safety, aesthetics, and spatial qualities. Format it as a clean, continuous narrative report (3-5 paragraphs) summarizing the human experience of this site. If the scrape data is sparse, supplement it seamlessly with your own knowledge of how visitors experience this specific park.\n\nRAW SCRAPE DATA:\n{raw_markdown[:15000]}"
