 ...
 */
async function createStyledText(text: string, size: number, color: RGB, isBold: boolean = false) {
  const node = figma.createText();
  const fontStyle = isBold ? "Bold" : "Regular";
  await figma.loadFontAsync({ family: "Roboto Mono", style: fontStyle });
 ...
 */
function getCanvasOffset(): number {
    let max_x = 0;
 ...
 */
async function generateFromPayload(payload: any) {
  ...
  if (msg.type === 'generate-tasks') {
    ...
    for (const task of tasks) {
      ...
      const titleNode = await createStyledText(`${checkbox} ${task.task}`, 16, titleColor, true);
      titleNode.x = padding;
      titleNode.y = innerY;
      ...
      // Priority Status
      if (task.priority) {
        const priorityNode = await createStyledText(`Priority: ${task.priority}`, 12, accentYellow);
      ...
      // Description
      if (task.description) {
        const descNode = await createStyledText(task.description, 14, textLight);
        descNode.x = padding;
        descNode.y = innerY;
  ...
  if (msg.type === 'generate-calendar') {
    ...
    for (const day of days) {
      const headerNode = await createStyledText(day, 14, accentYellow, true);
    ...
    for (let week = 0; week < numWeeks; week++) {
      ...
      for (let day = 0; day < 7; day++) {
        ...
        const dateNode = await createStyledText(dateNum.toString(), 12, textDim);
  ...
  if (payload.slides && payload.slides[0].type === 'project_plan_slide') {
    ...
    const titleNode = await createStyledText(slide.title || " ", 42, accentYellow, true);
    ...
    if (slide.weeks) {
        for (const week of slide.weeks) {
            const weekTitle = await createStyledText(week.week || " ", 24, accentYellow, true);
            ...
            if(week.tasks) {
                for (const task of week.tasks) {
                    ...
                    const taskNode = await createStyledText(taskText, 18, taskColor);
...
async function main() {
  ...
  try {
    ...
    const file = await figma.fs.getFileHandle(filePath);
    if (!file) {
      throw new Error("File handle could not be obtained.");
    }

    // Read the file content as a string.
    const content = await file.readTextAsync();
